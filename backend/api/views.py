...
def event_list(request):
    ...
    return Response({"resperpage": resPerPage, "data": serializer.data}, status=status.HTTP_200_OK)
...
def delete_events(request,pk):
    ...
    events.delete()
    return Response({"message": f" The event :{events.title} was deleted successfully"}, status=status.HTTP_204_NO_CONTENT)
...
def create_news(request):
    ...
    if serializer.is_valid():
        serializer.save()
        return Response({"message": "News created successfully", "data":serializer.data}, status=status.HTTP_201_CREATED)
    else:
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
